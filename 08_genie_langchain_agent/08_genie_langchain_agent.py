...
# MAGIC %md
# MAGIC # Topic 8: Simple Genie Agent with LangChain (without whl)
# MAGIC
# MAGIC ## Use Case
# MAGIC A **Sales Analytics Agent** that connects to a **Genie Space** for natural-language queries over sales data. The agent uses:
# MAGIC - **LangChain** with `ChatDatabricks` and tool-calling
# MAGIC - **Databricks Genie API** via `@tool` decorated `query_genie_space`
# MAGIC - **MLflow ResponsesAgent** with `to_chat_completions_input()` and `output_to_responses_items_stream()`
# MAGIC - **Databricks Claude Sonnet 4.5** as the LLM
# MAGIC
# MAGIC ## Architecture
# MAGIC - **LangChain** `create_tool_calling_agent` with `query_genie_space` as the tool
# MAGIC - **ChatDatabricks(endpoint="databricks-claude-sonnet-4-5")**
# MAGIC - **MLflow ResponsesAgent** implementing `predict()` and `predict_stream()`
# MAGIC - Evaluation, UC registration, deployment, and querying via DatabricksOpenAI
...
# MAGIC %md
# MAGIC ## 2. Imports and Genie Query Tool
...
# MAGIC %md
# MAGIC ## 3. Build LangChain Agent

# COMMAND ----------

SYSTEM_PROMPT = """You are a Sales Analytics assistant. You have access to a Genie space with sales data.
Use the query_sales_genie tool to answer questions about sales, revenue, products, regions, and trends.
...
class GenieSalesLangChainAgent(ResponsesAgent):
...
agent = GenieSalesLangChainAgent()
set_model(agent)
...
with mlflow.start_run(run_name="genie_langchain_sales_agent"):
...
# MAGIC Evaluate the agent on sales analytics questions using Safety and RelevanceToQuery scorers.
...
UC_MODEL_NAME = "sjdatabricks.agents.genie_langchain_sales"
...
endpoint_name = "genie-langchain-sales-agent"
...
# MAGIC Use the **DatabricksOpenAI** client to query the deployed agent via Responses API.
...
# MAGIC %md
# MAGIC ### Alternative: Chat Completions API

# MAGIC For endpoints that support Chat Completions format:
...
# MAGIC     model=model_for_inference,
# MAGIC     messages=input_msgs,
# MAGIC     max_tokens=512,
# MAGIC )
# MAGIC print(response.choices[0].message.content)
# MAGIC
# MAGIC # Streaming
# MAGIC stream = client.chat.completions.create(
# MAGIC     model=model_for_inference,
# MAGIC     messages=input_msgs,
# MAGIC     max_tokens=512,
# MAGIC     stream=True,
...
# MAGIC     if chunk.choices and chunk.choices[0].delta.content:
# MAGIC         print(chunk.choices[0].delta.content, end="")
# MAGIC ```

# COMMAND ----------

# MAGIC %md
# MAGIC ## 10. Local Testing (Before Deployment)

# MAGIC Test the agent locally. **Requires valid `SALES_GENIE_SPACE_ID`**.
...
print("Model set for MLflow. Genie LangChain agent ready for deployment.")
