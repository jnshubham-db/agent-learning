...
# MAGIC %md
# MAGIC # Deploy: DSPy + LangGraph Customer Support Agent (WHL)
# MAGIC
...
# MAGIC %md
# MAGIC ### 2a. Log Model (Models-from-Code)
# MAGIC
# MAGIC The `agent` module defines the model at import time via `set_model()`.
# MAGIC We log using the entry point path.
...
# MAGIC %md
# MAGIC ## 3. Evaluation with Safety and RelevanceToQuery
# MAGIC
...
model_info = mlflow.register_model(
    f"runs:/{run_id}/model",
    "sjdatabricks.agents.dspy_langgraph_support",
)
...
# MAGIC     name="dspy-langgraph-support-agent",
# MAGIC     config={
# MAGIC         "served_entities": [
# MAGIC             {
# MAGIC                 "name": "dspy-agent",
# MAGIC                 "entity_name": "sjdatabricks.agents.dspy_langgraph_support",
...
# MAGIC endpoint_name = "dspy-langgraph-support-agent"
# MAGIC
# MAGIC input_msgs = [{"role": "user", "content": "What is the status of order ORD-12345678?"}]
# MAGIC
...
# MAGIC
# MAGIC ```bash
# MAGIC uv pip install dist/dspy_langgraph_agent-0.1.0-py3-none-any.whl --system
# MAGIC ```
