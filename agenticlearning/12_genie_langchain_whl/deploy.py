...
# MAGIC %md
# MAGIC # Topic 12 — Deploy: Simple Genie Agent with LangChain (WHL)
# MAGIC
# MAGIC ## Overview
# MAGIC
# MAGIC This notebook deploys the **Genie LangChain Agent** packaged as a Python wheel.
# MAGIC
# MAGIC **Architecture:**
# MAGIC
# MAGIC | Component | Role |
# MAGIC |-----------|------|
# MAGIC | `genie.py` | `ChatDatabricks` LLM config, Genie Space query helper, `needs_order_data()` LLM-based router |
# MAGIC | `agent.py` | `GenieLangChainAgent(ResponsesAgent)` — routes via LLM classifier, calls Genie Space or falls back |
# MAGIC | `pyproject.toml` | Hatchling build config for the `genie_langchain_agent` wheel |
# MAGIC | `deploy.py` | *This notebook* — build, log, evaluate, register |
# MAGIC
# MAGIC **Flow:** User question ➜ LLM classifier (`needs_order_data`) ➜ Genie Space (data) *or* LLM fallback (general) ➜ ResponsesAgent response
# MAGIC
...
_sp.check_call([
    "uv", "pip", "install",
    "langchain", "langchain-core", "langchain-databricks",
    "mlflow>=3", "databricks-agents", "databricks-sdk", "pyspark",
    "--system",
])
...
# MAGIC
# MAGIC The `agent.py` module calls `set_model(GenieLangChainAgent())` at import time.
...
mlflow.set_experiment("/Users/{}/12_genie_langchain_whl".format(
    spark.sql("SELECT current_user()").first()[0]
))

with mlflow.start_run(run_name="genie_langchain_agent_whl") as run:
...
ENDPOINT_NAME = "genie-langchain-whl-agent"
